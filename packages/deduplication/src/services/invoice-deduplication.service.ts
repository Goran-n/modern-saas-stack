...
export class InvoiceDeduplicationService {
  ...
  private extractInvoiceData(
    ...
    return {
      vendorName:
        extractedFields.vendorName?.value ||
        extractedFields.supplierName?.value ||
        null,
      invoiceNumber:
        extractedFields.invoiceNumber?.value ||
        extractedFields.invoiceNo?.value ||
        null,
      invoiceDate:
        extractedFields.invoiceDate?.value ||
        extractedFields.date?.value ||
        null,
      totalAmount:
        extractedFields.totalAmount?.value ||
        extractedFields.total?.value ||
        null,
  ...
  async getDuplicateChain(extractionId: string): Promise<DocumentExtraction[]> {
    try {
      ...
      const chain = await this.db
        ...
        .where(
          eq(
            documentExtractions.invoiceFingerprint,
            extraction[0].invoiceFingerprint,
          ),
        )
