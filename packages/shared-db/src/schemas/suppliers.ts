...
export const dataSourceEnum = pgEnum("data_source", ["invoice", "manual"]);
...
export const suppliers = pgTable(
  ...
  (table) => ({
    ...
    companyNumberIdx: index("idx_suppliers_company_number").on(
      table.companyNumber,
    ),
    ...
    uniqueCompanyNumberPerTenant: uniqueIndex(
      "unique_company_number_per_tenant",
    )
    ...
    uniqueSlugPerTenant: uniqueIndex("unique_slug_per_tenant").on(
      table.tenantId,
      table.slug,
    ),
  }),
...
export const supplierDataSources = pgTable(
  ...
  (table) => ({
    supplierIdx: index("idx_supplier_data_sources_supplier_id").on(
      table.supplierId,
    ),
    sourceTypeIdx: index("idx_supplier_data_sources_source_type").on(
      table.sourceType,
    ),
    uniqueSourcePerSupplier: uniqueIndex("unique_source_per_supplier").on(
      table.supplierId,
      table.sourceType,
      table.sourceId,
    ),
...
export const supplierAttributes = pgTable(
  "supplier_attributes",
  {
    id: uuid("id").primaryKey().defaultRandom(),
    supplierId: uuid("supplier_id")
  ...
  (table) => ({
    supplierIdx: index("idx_supplier_attributes_supplier_id").on(
      table.supplierId,
    ),
    ...
    uniqueAttributeValue: uniqueIndex("unique_attribute_value").on(
      table.supplierId,
      table.attributeType,
      table.hash,
    ),
...
export const supplierDataSourcesRelations = relations(
  supplierDataSources,
  ({ one }) => ({
...
export const supplierAttributesRelations = relations(
  supplierAttributes,
  ({ one }) => ({
    supplier: one(suppliers, {
