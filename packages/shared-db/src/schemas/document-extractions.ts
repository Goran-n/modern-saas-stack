...
export const documentExtractions = pgTable('document_extractions', {
  ...
  documentTypeConfidence: numeric('document_type_confidence', { precision: 5, scale: 2 }).notNull(),
  
  // Extracted data with confidence
  extractedFields: jsonb('extracted_fields').notNull().$type<import('../types').ExtractedFields>(), // Field values with confidence scores
  companyProfile: jsonb('company_profile').$type<import('../types').CompanyProfile>(), // Standardised company data
  lineItems: jsonb('line_items').array().$type<import('../types').LineItem[]>(), // Array of line items
  
  // Quality metrics
  overallConfidence: numeric('overall_confidence', { precision: 5, scale: 2 }).notNull(),
  dataCompleteness: numeric('data_completeness', { precision: 5, scale: 2 }).notNull(),
