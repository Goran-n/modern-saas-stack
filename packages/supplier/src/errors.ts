...
export const createError = {
  // Identifier errors
  missingIdentifier: () => new SupplierError(
  ...
  duplicateCompanyNumber: (number: string) => new SupplierError(
  ...
  duplicateVatNumber: (number: string) => new SupplierError(
  ...
  // Supplier errors
  supplierNotFound: (id: string) => new SupplierError(
    'Supplier not found',
    'SUPPLIER_NOT_FOUND',
    404,
    undefined,
  ...
  invalidTenant: (tenantId: string) => new SupplierError(
    'Invalid tenant ID',
    'INVALID_TENANT',
    403,
    'tenantId',
  ...
  // Validation errors
  nameTooShort: (name: string) => new SupplierError(
  ...
  nameTooLong: (name: string) => new SupplierError(
    'Company name cannot exceed 200 characters',
    'NAME_TOO_LONG',
    400,
    'name',
    { provided: name.substring(0, 50) + '...', maxLength: 200, actualLength: name.length }
  ),
  
  nameNoLetters: (name: string) => new SupplierError(
  ...
  invalidCompanyNumber: (number: string, country: string) => new SupplierError(
  ...
  invalidVatNumber: (number: string, country: string) => new SupplierError(
  ...
  invalidEmail: (email: string) => new SupplierError(
    `Email address "${email}" appears to be invalid`,
    'INVALID_EMAIL',
    400,
    'email',
    { provided: email, suggestion: 'Please verify the email format (e.g., name@company.com)' }
  ),
  
  invalidPhone: (phone: string) => new SupplierError(
    `Phone number "${phone}" may be invalid`,
    'INVALID_PHONE',
    400,
    'phone',
    { provided: phone, suggestion: 'Ensure the phone number includes country code and has 7-20 digits' }
  ),
  
  invalidWebsite: (website: string) => new SupplierError(
    `Website URL "${website}" appears to be invalid`,
    'INVALID_WEBSITE',
    400,
    'website',
    { provided: website, suggestion: 'Please provide a valid URL (e.g., https://example.com)' }
  ),
};
