...
function cleanCompanyName(name: string): string {
  return name
    .toLowerCase()
    .replace(/\b(ltd|limited|plc|inc|incorporated|corp|corporation|llc|gmbh|ag|sa|srl|bv|nv|pty|pvt)\b/gi, '')
    .replace(/[^a-z0-9\s]/g, ' ')
    .replace(/\s+/g, ' ')
    .trim();
}
...
export function calculateAddressMatchScore(
  ...
  if (addr1.postcode && addr2.postcode) {
    factors++;
    if (addr1.postcode.replace(/\s+/g, '').toLowerCase() === 
  ...
  if (addr1.line1 && addr2.line1) {
    factors++;
    const streetSimilarity = calculateStringSimilarity(addr1.line1, addr2.line1);
