...
export class SupplierOperations {
  ...
  async triggerLogoFetchForGlobalSupplier(globalSupplierId: string) {
    try {
      ...
      if (!globalSupplier) {
        logger.debug("Global supplier not found", {
          globalSupplierId,
        });
        return;
      }
      ...
      if (!globalSupplier.primaryDomain) {
        logger.debug(
          "Global supplier has no domain, triggering domain discovery",
          {
            globalSupplierId,
          },
        );

        // Trigger domain discovery instead
        await tasks.trigger("domain-discovery", {
          globalSupplierIds: [globalSupplierId],
        });

        logger.info("Triggered domain discovery for global supplier", {
          globalSupplierId,
        });
        return;
      }
      ...
      logger.info(
        "Triggered logo fetch and website analysis for global supplier",
        {
          globalSupplierId,
      ...
      logger.error("Failed to trigger enrichment for global supplier", {
        globalSupplierId,
        error,
      });
