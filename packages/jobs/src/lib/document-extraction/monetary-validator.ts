...
export class MonetaryValidator {
  ...
  static validate(doc: AccountingDocument): MonetaryValidationResult {
    ...
    if (doc.totalAmount !== null) {
      if (doc.totalAmount < this.MIN_AMOUNT) {
        errors.push('Total amount cannot be negative');
      }
      if (doc.totalAmount > this.MAX_REASONABLE_AMOUNT) {
    ...
    if (doc.subtotalAmount !== null) {
      if (doc.subtotalAmount < this.MIN_AMOUNT) {
        errors.push('Subtotal amount cannot be negative');
      }
      if (doc.subtotalAmount > this.MAX_REASONABLE_AMOUNT) {
    ...
    if (doc.taxAmount !== null) {
      if (doc.taxAmount < this.MIN_AMOUNT) {
    ...
    if (doc.totalAmount !== null && doc.subtotalAmount !== null && doc.taxAmount !== null) {
      ...
      if (difference > this.ROUNDING_TOLERANCE) {
        errors.push(`Total (${doc.totalAmount}) does not equal subtotal (${doc.subtotalAmount}) + tax (${doc.taxAmount}). Difference: ${difference.toFixed(2)}`);
      }
    }

    // Validate currency code
    if (doc.currency !== null) {
      if (!this.isValidCurrencyCode(doc.currency)) {
    ...
    if (doc.lineItems && doc.lineItems.length > 0) {
      const lineItemsValidation = this.validateLineItems(doc.lineItems, doc.subtotalAmount);
  ...
  private static validateLineItems(
    ...
    lineItems.forEach((item, index) => {
      ...
      if (item.quantity !== null && item.unitPrice !== null && item.totalPrice !== null) {
        ...
        if (difference > this.ROUNDING_TOLERANCE) {
          warnings.push(`Line item ${index + 1}: Calculated total (${calculatedPrice.toFixed(2)}) does not match stated total (${item.totalPrice})`);
        }
      }
    });
    ...
    if (documentSubtotal !== null && calculatedSubtotal > 0) {
      ...
      if (difference > this.ROUNDING_TOLERANCE) {
        warnings.push(`Line items total (${calculatedSubtotal.toFixed(2)}) does not match document subtotal (${documentSubtotal})`);
      }
    }
  ...
  private static isValidCurrencyCode(code: string): boolean {
    ...
    const validCodes = [
      ...
      'HRK', 'RUB', 'TRY', 'BRL', 'MXN', 'INR', 'IDR', 'KRW',
      'CNY', 'HKD', 'SGD', 'THB', 'MYR', 'PHP', 'ZAR', 'EGP',
      'AED', 'SAR', 'QAR', 'KWD', 'BHD', 'OMR', 'JOD', 'ILS',
      'TWD', 'ARS', 'CLP', 'COP', 'PEN', 'UYU', 'PKR', 'LKR',
      'NGN', 'KES', 'GHS', 'MAD', 'TND', 'DZD', 'VND', 'UAH'
    ];
