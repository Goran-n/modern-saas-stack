...
export class TextExtractor {
  ...
  async extractFromUrl(fileUrl: string, mimeType: string): Promise<string> {
    ...
    if (contentLength && parseInt(contentLength) > this.MAX_SIZE) {
      throw new Error(`File too large: ${(parseInt(contentLength) / 1024 / 1024).toFixed(2)}MB exceeds 10MB limit`);
    }
    ...
    if (content.byteLength > this.MAX_SIZE) {
      throw new Error(`File too large: ${(content.byteLength / 1024 / 1024).toFixed(2)}MB exceeds 10MB limit`);
    }
    
    return this.extractFromBuffer(content, mimeType);
  }

  async extractFromBuffer(content: ArrayBuffer, mimeType: string): Promise<string> {
    let extractedText = '';
    ...
    if (mimeType === 'application/pdf' || mimeType === 'application/octet-stream') {
      ...
      logger.warn('Image files require OCR processing, which is not yet implemented', { mimeType });
