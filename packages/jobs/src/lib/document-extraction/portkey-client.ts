...
export function getPortkeyClient(): Portkey {
  if (!portkeyInstance) {
    ...
    if (!config.PORTKEY_API_KEY) {
      logger.error('PORTKEY_API_KEY is not configured. Document extraction will not work.');
      throw new Error('PORTKEY_API_KEY is required for document extraction. Please run: doppler secrets set PORTKEY_API_KEY=<your-key>');
    }
    
    if (!config.PORTKEY_VIRTUAL_KEY) {
      logger.error('PORTKEY_VIRTUAL_KEY is not configured. Document extraction will not work.');
      throw new Error('PORTKEY_VIRTUAL_KEY is required for document extraction. Please run: doppler secrets set PORTKEY_VIRTUAL_KEY=<your-key>');
    }
...
export function createPortkeyAnthropicModel() {
  ...
  return {
    ...
    defaultHeaders: {
      ...
      ...(portkey.virtualKey && { 'x-portkey-virtual-key': portkey.virtualKey }),
    },
  };
}