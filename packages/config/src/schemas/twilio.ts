...
export const twilioSchema = z.object({
   ...
   */
  TWILIO_WHATSAPP_NUMBER: z.string().regex(/^\+\d+$/).describe('Twilio WhatsApp number in E.164 format'),
  
  /**
   * Webhook URL (optional)
   * Used for reference/documentation, actual webhook URL is set in Twilio Console
   */
  TWILIO_WEBHOOK_URL: z.string().url().optional().describe('Webhook URL configured in Twilio'),
  
  /**
   * WhatsApp Webhook Verification Token (optional)
   * Used to verify webhook requests are from Twilio
   */
  WHATSAPP_VERIFY_TOKEN: z.string().optional().default('kibly-whatsapp-verify').describe('Token for webhook verification'),
});
