...
export const slackSchema = z.object({
  ...
  SLACK_SIGNING_SECRET: z
    ...
    .optional()
    .describe(
      "Slack Signing Secret for webhook verification (required for Slack integration)",
    ),
   ...
   * Used for reference
   */
  SLACK_APP_ID: z.string().optional().describe("Slack App ID"),

  /**
   * Slack Client ID (optional)
   * Used for OAuth flows
   */
  SLACK_CLIENT_ID: z.string().optional().describe("Slack OAuth Client ID"),
