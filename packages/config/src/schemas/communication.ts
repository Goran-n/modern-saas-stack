...
export const communicationServiceSchema = z.object({
   ...
   */
  COMMUNICATION_MAX_FILE_SIZE: z.coerce.number()
    .int()
   ...
   */
  COMMUNICATION_WHATSAPP_MIME_TYPES: z.string()
    .default('application/pdf,image/png,image/jpeg,image/jpg')
   ...
   */
  COMMUNICATION_SLACK_MIME_TYPES: z.string()
    .default('application/pdf')
    .transform(val => val.split(',').map(s => s.trim()))
   ...
   * @default 'application/pdf'
   */
  COMMUNICATION_DEFAULT_MIME_TYPE: z.string()
    .default('application/pdf')
   ...
   * @default 'whatsapp:'
   */
  COMMUNICATION_WHATSAPP_PREFIX: z.string()
    .default('whatsapp:')
    .describe('Prefix to add to phone numbers for WhatsApp'),
});

export type CommunicationServiceConfig = z.infer<typeof communicationServiceSchema>;