...
export async function createTenant(input: CreateTenantInput): Promise<{ tenant: Tenant; member: TenantMember }> {
...
export async function updateTenant(input: UpdateTenantInput): Promise<Tenant> {
  ...
  if (validated.subscription !== undefined) updates.subscription = validated.subscription
