...
import { users, tenantMembers, invitations } from '@kibly/shared-db/schemas/tenants';
import { eq, and, isNull } from 'drizzle-orm';
import { nanoid } from 'nanoid';
import { v4 as uuidv4 } from 'uuid';
import { addDays } from 'date-fns';
import { inviteMemberSchema, updateMemberRoleSchema, createUserSchema } from '../types';
...
export async function inviteMember(input) {
    ...
    if (existingUser) {
        const [existingMember] = await db.select()
            .from(tenantMembers)
            .where(and(eq(tenantMembers.tenantId, validated.tenantId), eq(tenantMembers.userId, existingUser.id)))
            .limit(1);
        if (existingMember) {
...
export async function updateMemberRole(input) {
    ...
    if (validated.role !== 'owner') {
        const owners = await db.select()
            .from(tenantMembers)
            .where(and(eq(tenantMembers.tenantId, validated.tenantId), eq(tenantMembers.role, 'owner')));
    ...
    const [updated] = await db.update(tenantMembers)
        ...
        .where(and(eq(tenantMembers.id, validated.memberId), eq(tenantMembers.tenantId, validated.tenantId)))
        .returning();
    if (!updated) {
...
//# sourceMappingURL=member-actions.js.map