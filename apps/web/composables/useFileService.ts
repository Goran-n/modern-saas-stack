...
export interface FileServiceError {
  code: 'NETWORK_ERROR' | 'PERMISSION_DENIED' | 'FILE_NOT_FOUND' | 'PROCESSING_ERROR'
  message: string
  details?: unknown
}
...
export const useFileService = () => {
  ...
  // Loading states
  const isOperationInProgress = (fileId: string, type?: FileOperation['type']) => {
  ...
  // Error handling
  const handleError = (error: unknown, code: FileServiceError['code']): FileServiceError => {
  ...
  // Track operation
  const trackOperation = (fileId: string, type: FileOperation['type']): FileOperation => {
  ...
  const handleFileDragStart = (event: DragEvent, file: FileItem, tenantId?: string) => {
  ...
  const activeOperations = computed(() => 
    Array.from(operations.value.values()).filter(op => op.status === 'in_progress')
  )
