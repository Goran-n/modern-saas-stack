/**
 * Content script for Kibly pages
 * Detects when files are being dragged and notifies the extension
 */
import { defineContentScript } from 'wxt/utils/define-content-script';
import { sendToBackground } from '../utils/messaging';
import { MessageType, FileDragPayload } from '../types/messages';
import { createLogger } from '../utils/logger';

export default defineContentScript({
  matches: ['https://*.kibly.com/*', 'http://localhost:*/*'],
  
  main() {
    const logger = createLogger('content:kibly');
    logger.info('Kibly content script loaded');
    ...
    DataTransfer.prototype.setData = function(format: string, data: string) {
      ...
      if (format === 'application/x-kibly-file') {
        // Convert custom MIME type to text/plain
        originalSetData.call(this, 'text/plain', data);
        
        try {
          const parsedData = JSON.parse(data);
          if (parsedData.type === 'kibly-file') {
            currentDragData = parsedData;
            logger.debug('Intercepted Kibly file drag data (converted to text/plain)', { 
        ...
        if (format === 'text/plain') {
          try {
            ...
            if (parsedData.type === 'kibly-file') {
              currentDragData = parsedData;
              logger.debug('Intercepted Kibly file drag data', { 
                fileId: parsedData.fileId,
                fileName: parsedData.fileName,
                format: format
              });
    ...
    document.addEventListener('dragstart', async (event) => {
      ...
      setTimeout(async () => {
        if (!currentDragData || currentDragData.type !== 'kibly-file') {
          return;
        }

        try {
          logger.debug('Processing Kibly file drag', { 
    ...
    window.addEventListener('message', async (event) => {
      ...
      if (event.data?.type === 'kibly-file-copied') {
        ...
        // Store in Chrome storage for the extension
        await chrome.storage.local.set({
          copiedFileData: fileData
        });
      ...
      try {
        ...
        if (supabaseAuth) {
          try {
            ...
            authToken = authData.access_token || authData.currentSession?.access_token;
          } catch (e) {
        ...
        if (tenantId || authToken) {
          ...
          logger.info('Stored auth info', { hasTenantId: !!tenantId, hasAuthToken: !!authToken });
        }
