...
export default {
  async main() {
    ...
    try {
      ...
      const authRejected =
        authSuccess === "false" || urlParams.get("error") === "access_denied";
      ...
      if (hasAuthSuccess) {
        ...
        if (!sessionData) {
          try {
            ...
            logger.error("Failed to get session data from sessionStorage", {
              error,
            });
          }
        }
        ...
        chrome.runtime.sendMessage(
          createMessage(MessageType.AUTH_SUCCESS, {
            method: "web_app",
            timestamp: Date.now(),
            sessionData,
          }),
