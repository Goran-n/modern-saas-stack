          ...
          switch (message.type) {
            case MessageType.FILE_DOWNLOAD_REQUEST:
              return await handleFileDownload(
                message.payload as FileDownloadRequestPayload,
              );
        ...
        try {
          ...
          const results = await chrome.scripting.executeScript({
            ...
            func: () => {
              ...
              const sessionResult = window.sessionStorage.getItem(
                "figgy_extension_auth_result",
              );
          ...
          if (authResult) {
            ...
            logger.info("Auth result detected via polling", {
        ...
        if (payload.sessionData) {
          try {
            ...
            try {
              ...
              logger.warn(
                "Supabase setSession failed, using manual storage approach",
                { error: supabaseError },
              );
              ...
              try {
                ...
                logger.warn("Could not refresh session after manual storage", {
                  error: refreshError,
                });
              }
            }
    ...
    async function handleFileDownload(
      ...
      try {
        ...
        try {
          ...
          logger.warn(
            "Failed to get session, user may need to re-authenticate",
            { error: sessionError },
          );
          throw new Error(
            "Authentication failed. Please log in to Figgy again.",
          );
        }
        ...
        logger.debug("Starting in-memory file fetch", {
          fileName: payload.fileName,
        });
