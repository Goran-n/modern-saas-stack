...
import { getDatabaseConnection, checkDatabaseHealth } from "@kibly/shared-db";

async function waitForDatabase(maxRetries = 10, retryDelay = 2000): Promise<boolean> {
  for (let i = 0; i < maxRetries; i++) {
    ...
    try {
      ...
      logger.warn(`Database connection check failed (attempt ${i + 1})`, { 
        error: errorMessage,
        isNetworkError: errorMessage.includes('CONNECT_TIMEOUT') || errorMessage.includes('ECONNREFUSED'),
      });
    }
...
async function main() {
  ...
  if (skipDbCheck) {
    ...
    if (!isDatabaseReady) {
      logger.error("Failed to establish database connection after multiple retries");
      logger.info("💡 Hint: You can skip database checks by setting SKIP_DB_CHECK=true");
      process.exit(1);
    }
  }
