...
async function main() {
  try {
    ...
    if (!tenantId || !integrationId) {
      logger.error(
        "Usage: bun run test-full-sync.ts <tenantId> <integrationId> [entities]",
      );
      logger.info("Example: bun run test-full-sync.ts tenant123 int456");
      logger.info(
        "Example with specific entities: bun run test-full-sync.ts tenant123 int456 contacts,accounts",
      );
      logger.info(
        "Example with all entities: bun run test-full-sync.ts tenant123 int456 all",
      );
      process.exit(1);
    }
    ...
    const tenantMemberRepository = new DrizzleTenantMemberRepository(
      drizzleClient,
    );
